...
class G1PrepareEvacuationTask : public WorkerTask {
  class G1PrepareRegionsClosure : public G1HeapRegionClosure {
    ...
    bool humongous_region_is_candidate(G1HeapRegion* region) const {
      ...
      // that is in the mark stack.  That code also ensures that metadata is kept live.
      //
      ...
      // reclaimed objects were already allocated into agaon.
...
<<<<<<< HEAD
<<<<<<< HEAD
      if (!obj->is_array() || _g1h->array_has_oops(obj)) {
=======
      if (G1CollectedHeap::array_has_oops(obj)) {
>>>>>>> 73d48eec243 (* initial version)
=======
      if (obj->is_instance() || _g1h->array_has_oops(obj)) {
>>>>>>> 14a4d6d1710 (* fix issue with large instanceoops)
